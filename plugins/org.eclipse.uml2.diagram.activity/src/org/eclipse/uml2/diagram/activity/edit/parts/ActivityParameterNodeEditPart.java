...
public class ActivityParameterNodeEditPart extends AbstractBorderItemEditPart implements PrimaryShapeEditPart {
	...
	protected NodeFigure createNodePlate() {
		DefaultSizeNodeFigure result = new DefaultSizeNodeFigure(getMapMode().DPtoLP(70), getMapMode().DPtoLP(25));
	...
	protected org.eclipse.gef.EditPart chooseLabelEditPartForDirectEditRequest(Request request) {
		if (request.getExtendedData().containsKey(RequestConstants.REQ_DIRECTEDIT_EXTENDEDDATA_INITIAL_CHAR)) {
			...
			if (initialChar.charValue() == '<') {
				org.eclipse.gef.EditPart result = getChildBySemanticHint(UMLVisualIDRegistry.getType(ActivityParameterNodeStereotypeEditPart.VISUAL_ID));
				if (result != null) {
					return result;
				}
			}
