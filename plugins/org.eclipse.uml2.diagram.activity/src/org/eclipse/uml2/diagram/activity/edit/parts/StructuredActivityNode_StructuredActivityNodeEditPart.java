...
public class StructuredActivityNode_StructuredActivityNodeEditPart extends ShapeNodeEditPart implements PrimaryShapeEditPart {
	...
	private List<? extends IUpdaterLinkDescriptor> getStructuredActivityNode_3009ContainedLinks() {
	...
	public EditPart getTargetEditPart(Request request) {
		if (request instanceof CreateUnspecifiedTypeRequest) {
			...
			if (types.contains(UMLElementTypes.StructuredActivityNode_3009)) {
				return getChildBySemanticHint(UMLVisualIDRegistry.getType(StructuredActivityNode_StructuredActivityNodeContentPaneCompartmentEditPart.VISUAL_ID));
			}
			...
			if (types.contains(UMLElementTypes.ConditionalNode_3092)) {
				return getChildBySemanticHint(UMLVisualIDRegistry.getType(StructuredActivityNode_StructuredActivityNodeContentPaneCompartmentEditPart.VISUAL_ID));
			}
			if (types.contains(UMLElementTypes.InitialNode_3093)) {
				return getChildBySemanticHint(UMLVisualIDRegistry.getType(StructuredActivityNode_StructuredActivityNodeContentPaneCompartmentEditPart.VISUAL_ID));
			}
		}
