...
public class PortProvidedCreateCommand extends EditElementCommand {
	...
	public PortProvidedCreateCommand(CreateRelationshipRequest request, EObject source, EObject target) {
		...
		this.source = source;
		this.target = target;
		myCreateLinkHelper = new ProvidedPortLinkHelper((AdapterFactoryEditingDomain) getEditingDomain(), source, target, UMLDiagramEditorPlugin.getInstance().getPreferenceStore());
	}
	...
	public boolean canExecuteGen() {
		...
		return UMLBaseItemSemanticEditPolicy.getLinkConstraints().canCreatePortProvided_4006(getSource(), getTarget());
	}
