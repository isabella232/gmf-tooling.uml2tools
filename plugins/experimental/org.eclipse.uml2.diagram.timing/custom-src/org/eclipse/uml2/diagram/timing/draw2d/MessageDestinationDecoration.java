...
public class MessageDestinationDecoration extends CompositeDecoration {
	...
	public void update(DMessage message, IPreferenceStore store) {
		Message umlMessage = message.getUmlMessage();
		MessageSort sort = umlMessage == null ? MessageSort.SYNCH_CALL_LITERAL : umlMessage.getMessageSort();
		updateMessageSort(sort);
		updateReply(sort);
	}
