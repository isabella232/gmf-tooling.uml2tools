...
public class PortRequiredReorientCommand extends EditElementCommand {
	...
	public PortRequiredReorientCommand(ReorientReferenceRelationshipRequest request) {
		...
		myLinkHelper = new RequiredPortLinkHelper((AdapterFactoryEditingDomain) getEditingDomain(), referenceOwner, oldEnd, UMLDiagramEditorPlugin.getInstance().getPreferenceStore());
	}
	...
	protected boolean canReorientSourceGen() {
		...
		return UMLBaseItemSemanticEditPolicy.getLinkConstraints().canExistPortRequired_4018(getNewSource(), getOldTarget());
	}
	...
	protected boolean canReorientTargetGen() {
		...
		return UMLBaseItemSemanticEditPolicy.getLinkConstraints().canExistPortRequired_4018(getOldSource(), getNewTarget());
	}
