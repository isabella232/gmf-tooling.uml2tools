...
public class ChangeDependencyTypeTest extends ClassDiagramCreationToolTest {
	...
	private void runChangeDependencyTypeAction(EClass type) {
		...
		DiagramAction action = new ChangeDependencyType(getDiagramWorkbenchPart().getSite().getPage(), type, "testID");
		action.init();
		assertTrue("Action should be enabled in correct cases: DependencyType: " + oursDependencyTypes.get(myIndex1).getName() + ", Actiontype: " + oursDependencyTypes.get(myIndex1).getName(), action.isEnabled());
		action.run();
	}
